...
# 绘制模型与PDE精确解之间的误差
# 生成输入数据 (x, z)
x_vals = np.linspace(-x_upper, x_upper, 101)  # 在 [-L, L] 范围内生成 100 个点
z_vals = np.linspace(z_lower, z_upper, 101)  # 在 [0, Z] 范围内生成 100 个点
...
# 保存为 CSV 文件（XYZ 格式）
csv_file_path = "./error_data_xyz.csv"
df_xyz.to_csv(csv_file_path, index=False)
