...
# 加载数据
u_exact_real_data = scipy.io.loadmat('../data/u_real_n1.mat')['data_noisy']
u_exact_imag_data = scipy.io.loadmat('../data/u_imag_n1.mat')['data_noisy']
...
# 创建PDE网格点
x = np.linspace(x_lower, x_upper, num_x * 2)
z = np.linspace(z_lower, z_upper, num_z * 2)
xx, zz = np.meshgrid(x, z, indexing='xy')  # 创建网格点
xx = torch.tensor(xx.reshape((-1, 1)), dtype=torch.float32, device=device)
zz = torch.tensor(zz.reshape((-1, 1)), dtype=torch.float32, device=device)
