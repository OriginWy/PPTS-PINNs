...
def test_and_plot(model, epoch):
    ...
    plt.title(f'inverse15_5 Comparison of $V_{{pred}}$ and $V_{{pt}}(x)$ (Real Part) - Epoch {epoch}')
    plt.xlabel('$x$')
    plt.ylabel('Value')
    plt.legend()
    plt.grid()
    ...
    plt.title(f'inverse15_5 Comparison of $W_{{pred}}$ and $V_{{pt}}(x)$ (Imaginary Part) - Epoch {epoch}')
    plt.xlabel('$x$')
    plt.ylabel('Value')
    plt.legend()
    plt.grid()
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % log_interval == 0 or epoch == 0:
        ...
        # 保存训练好的模型
        torch.save(model, '../model15/model15_5.pth')
#
#         将当前损失值记录到列表中
#         loss_data.append({
#             'epoch': epoch + 1,
...
print("训练完成，模型已保存为 model15_5.pth。")
...
plt.colorbar(cp)
plt.title('inverse15_5 Error between PINNs and actual function $u(x,z)$')
plt.xlabel('x')
plt.ylabel('z')
plt.show()
