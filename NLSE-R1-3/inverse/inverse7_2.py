...
# 加载数据
u_exact_real_data = scipy.io.loadmat('../data/u_real_n2.mat')['data_noisy']
u_exact_imag_data = scipy.io.loadmat('../data/u_imag_n2.mat')['data_noisy']
...
# 创建PDE网格点
x = np.linspace(x_lower, x_upper, num_x * 3)
z = np.linspace(z_lower, z_upper, num_z * 3)
xx, zz = np.meshgrid(x, z, indexing='xy')  # 创建网格点
xx = torch.tensor(xx.reshape((-1, 1)), dtype=torch.float32, device=device)
zz = torch.tensor(zz.reshape((-1, 1)), dtype=torch.float32, device=device)

# 展平后合并
input_xz_pde = torch.cat((xx, zz), dim=1)  # [num_points, 2]
input_xz_pde = torch.cat((input_xz_pde, input_xz), dim=0)
input_xz_pde.requires_grad_(True)


# 内部观测数据损失函数定义
def charbonnier_loss(pred, target, epsilon=1e-6):
    """
    Charbonnier 损失函数（平滑的 L1 损失）。

    参数:
    - pred: 预测值
    - target: 目标值
    - epsilon: 平滑参数，默认为 1e-6

    返回:
    - 计算的 Charbonnier 损失
    """
    error = pred - target
    loss = torch.sqrt(error ** 2 + epsilon ** 2)
    return torch.mean(loss)


noise_level = 0.5
real_noise_mean = noise_level * (torch.mean(torch.abs(u_exact_real)).item())
imag_noise_mean = noise_level * (torch.mean(torch.abs(u_exact_imag)).item())


def loss_u_actual(net, input_xz, u_exact_real, u_exact_imag, loss_u_weight=1, use_charbonnier=False):
    """
    ...
    - loss_u_weight: 损失的权重系数，默认值为 0.2
    - use_log_cosh: 是否使用 Log-Cosh 损失，默认为 False（使用 MSE 损失）
    ...
    if use_charbonnier:
        # 使用外部定义的 Log-Cosh 损失
        loss_real = charbonnier_loss(p, u_exact_real, epsilon=real_noise_mean)  # 实部损失
        loss_imag = charbonnier_loss(q, u_exact_imag, epsilon=imag_noise_mean)  # 虚部损失
    else:
        # 使用 PyTorch 内置 MSE 损失
        mse_loss = nn.MSELoss()
        loss_real = mse_loss(p, u_exact_real)  # 实部损失
        loss_imag = mse_loss(q, u_exact_imag)  # 虚部损失

    # 返回加权损失
    return loss_u_weight * (loss_real + loss_imag)
...
# 读取.mat文件
data = scipy.io.loadmat('../data/pt_left.mat')
...
def pde_losses(x, u, pde_weight=1):
    ...
    pde_real = -u_imag_z + u_real_xx + (u_real ** 2 + u_imag ** 2) * u_real + V * u_real - W * u_imag
    pde_imag = u_real_z + u_imag_xx + (u_real ** 2 + u_imag ** 2) * u_imag + V * u_imag + W * u_real
    ...
    # return pde_weight * (pde_loss + gradients_loss)
...
def test_and_plot(model, epoch):
    ...
    plt.title(f'inverse7_2 Comparison of $V_{{pred}}$ and $V_{{pt}}(x)$ (Real Part) - Epoch {epoch}')
    plt.xlabel('$x$')
    plt.ylabel('Value')
    plt.legend()
    plt.grid()
    ...
    plt.title(f'inverse7_2 Comparison of $W_{{pred}}$ and $V_{{pt}}(x)$ (Imaginary Part) - Epoch {epoch}')
    plt.xlabel('$x$')
    plt.ylabel('Value')
    plt.legend()
    plt.grid()
...
# 定义优化器和学习率调度器
optimizer1 = optim.AdamW(model.subnet1.parameters(), lr=learning_rate, weight_decay=0)
optimizer2 = optim.AdamW(model.subnet2.parameters(), lr=learning_rate, weight_decay=0)
optimizer3 = optim.AdamW(model.subnet3.parameters(), lr=learning_rate, weight_decay=0)

# 定义余弦退火学习率调度器
# T_max 是余弦退火的周期（单位：epoch），eta_min 是最小学习率
# scheduler = CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=100 * 1e-6)
...
for epoch in range(num_epochs):
    ...
    # PDE损失
    u_pred = model(input_xz_pde)  # 网络预测

    # loss_p = pde_losses(input_xz_pde, u_pred)
    loss_p, loss_g = pde_losses(input_xz_pde, u_pred)

    # 总损失
    # total_loss = loss_u + loss_b + loss_i + loss_p
    total_loss = loss_u + loss_b + loss_i + loss_p + loss_g
    ...
    scheduler2.step(loss_p.item())  # 基于 subnet2 的损失更新
    scheduler3.step(loss_p.item())  # 基于 subnet3 的损失更新

    # 日志打印
    if (epoch + 1) % log_interval == 0 or epoch == 0:
        current_lr1 = scheduler1.get_last_lr()[0]  # 获取当前学习率
        current_lr2 = scheduler2.get_last_lr()[0]
        current_lr3 = scheduler3.get_last_lr()[0]
        print(f"Epoch [{epoch + 1}/{num_epochs}], "
              f"Loss_u: {loss_u.item():.5e}, "
              f"Loss_b: {loss_b.item():.5e}, "
              f"Loss_i: {loss_i.item():.5e}, "
              f"Loss_p: {loss_p.item():.5e}, "
              f"Loss_g: {loss_g.item():.5e}, "
              f"Total Loss: {total_loss.item():.5e}, "
              f"LR1: {current_lr1:.5e}, "
              f"LR2: {current_lr2:.5e}, "
              f"LR3: {current_lr3:.5e}")

        # 每 1000 次迭代测试并绘图
        test_and_plot(model, epoch + 1)

        # 保存训练好的模型
        torch.save(model, '../model7/model7_2.pth')

# 定义 L-BFGS 优化器
lbfgs_optimizer = optim.LBFGS(model.parameters(), lr=learning_rate, max_iter=500, history_size=100)
num_epochs = 0
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % log_interval == 0:
        ...
        input_xz_pde_eval = input_xz_pde.detach().requires_grad_(True)
        u_pred_pde = model(input_xz_pde_eval)
        loss_p = pde_losses(input_xz_pde_eval, u_pred_pde)
        total_loss = loss_u + loss_b + loss_i + loss_p
...
print("训练完成，模型已保存为 model7_2.pth。")
...
plt.colorbar(cp)
plt.title('inverse7_2 Error between PINNs and actual function $u(x,z)$')
plt.xlabel('x')
plt.ylabel('z')
plt.show()
