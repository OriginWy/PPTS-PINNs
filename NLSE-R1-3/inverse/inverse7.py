...
def log_cosh_loss(pred, target):
    """
    Log-Cosh 损失函数：适用于对大误差具有鲁棒性的损失计算。
    """
    return torch.mean(torch.log(torch.cosh(pred - target)))


def loss_u_actual(net, input_xz, u_exact_real, u_exact_imag, loss_u_weight=1, use_log_cosh=False):
    """
    ...
    if use_log_cosh:
        # 使用外部定义的 Log-Cosh 损失
        loss_real = log_cosh_loss(p, u_exact_real)  # 实部损失
        loss_imag = log_cosh_loss(q, u_exact_imag)  # 虚部损失
    else:
...
def test_and_plot(model, epoch):
    ...
    plt.title(f'inverse7 Comparison of $V_{{pred}}$ and $V_{{pt}}(x)$ (Real Part) - Epoch {epoch}')
    plt.xlabel('$x$')
    plt.ylabel('Value')
    plt.legend()
    plt.grid()
    ...
    plt.title(f'inverse7 Comparison of $W_{{pred}}$ and $V_{{pt}}(x)$ (Imaginary Part) - Epoch {epoch}')
    plt.xlabel('$x$')
    plt.ylabel('Value')
    plt.legend()
    plt.grid()
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % log_interval == 0 or epoch == 0:
        ...
        # 保存训练好的模型
        torch.save(model, '../model7/model7.pth')
...
plt.colorbar(cp)
plt.title('inverse7 Error between PINNs and actual function $u(x,z)$')
plt.xlabel('x')
plt.ylabel('z')
plt.show()
