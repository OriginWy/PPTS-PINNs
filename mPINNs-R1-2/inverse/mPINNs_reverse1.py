...
def pde_losses(x, u, pde_weight=1):
    ...
    V_z = compute_gradient(V, x)[:, 1:2]
    W_z = compute_gradient(W, x)[:, 1:2]
    V_grad_loss = torch.mean(V_z ** 2) + torch.mean(W_z ** 2)
...
def test_and_plot(model, epoch):
    ...
    plt.title(f'mPINNs Comparison of $V_{{pred}}$ and $V_{{pt}}(x)$ (Real Part) - Epoch {epoch}')
    plt.xlabel('$x$')
    plt.ylabel('Value')
    plt.legend()
    plt.grid()
    ...
    plt.title(f'mPINNs Comparison of $W_{{pred}}$ and $V_{{pt}}(x)$ (Imaginary Part) - Epoch {epoch}')
    plt.xlabel('$x$')
    plt.ylabel('Value')
    plt.legend()
    plt.grid()
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % log_interval == 0 or epoch == 0:
        ...
        # 保存训练好的模型
        torch.save(model, '../mPINNs_model/model.pth')
...
plt.colorbar(cp)
plt.title('mPINNs Error between PINNs and actual function $u(x,z)$')
plt.xlabel('x')
plt.ylabel('z')
plt.show()
